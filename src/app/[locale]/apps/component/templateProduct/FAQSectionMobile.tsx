...
const FAQSectionMobile: React.FC<FAQSectionMobileProps> = ({
  ...
  return (
    <section className="w-full bg-white flex flex-col items-center py-12 px-4 sm:hidden">
      <h2 className="text-h3 text-[#333333] text-center mb-6">
        {title}
      </h2>
      <div className="flex flex-col gap-4 w-full max-w-[430px] mx-auto">
        {faqs.map((faq, idx) => (
          <div key={idx} className="w-full">
            <button
              className={`flex justify-between items-center w-full bg-[#F9F9F9] rounded-xl px-6 py-4 min-h-[84px] transition-shadow duration-200 focus:outline-none ${openIndex === idx ? "shadow-lg" : ""}`}
              onClick={() => setOpenIndex(openIndex === idx ? null : idx)}
              aria-expanded={openIndex === idx}
              aria-controls={`faq-answer-mobile-${idx}`}
              aria-label={`Pregunta: ${faq.question}`}
            >
              <span className="text-lg font-medium text-[#333333] text-left pr-3 leading-[1.5]">
                {faq.question}
              </span>
